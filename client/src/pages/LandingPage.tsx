...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-emerald-50/30 to-white">
      ...
      {/* Banner Promocional - Campeonato Grátis */}
      {promoBannerVisible && (
        <div className="fixed top-16 left-0 right-0 z-40 shadow-2xl" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 40%, #0f172a 100%)' }}>
          {/* Borda brilhante animada no topo */}
          <div className="h-1 w-full" style={{ background: 'linear-gradient(90deg, #fbbf24, #f59e0b, #eab308, #fbbf24, #f59e0b)' }} />
          <div className="container py-4 md:py-5 px-4">
            <div className="flex flex-col md:flex-row items-center justify-between gap-3 md:gap-6">
              {/* Texto principal */}
              <div className="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
                <div className="flex-shrink-0 w-12 h-12 md:w-14 md:h-14 rounded-full bg-yellow-400 flex items-center justify-center shadow-lg shadow-yellow-400/30">
                  <span className="text-2xl md:text-3xl">⚽</span>
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="inline-block bg-red-500 text-white text-[10px] md:text-xs font-black px-2 py-0.5 rounded uppercase tracking-wider animate-pulse">Vagas Limitadas</span>
                  </div>
                  <p className="text-white font-extrabold text-base md:text-xl lg:text-2xl leading-tight">
                    Seu campeonato organizado <span className="text-yellow-400">100% de graça!</span>
                  </p>
                  <p className="text-gray-400 text-xs md:text-sm leading-tight mt-0.5">
                    Estamos selecionando campeonatos para usar o PeladaPro sem custo. Fale conosco!
                  </p>
                </div>
              </div>
              {/* Botão WhatsApp grande + fechar */}
              <div className="flex items-center gap-3 flex-shrink-0">
                <a
                  href={`https://wa.me/551151981694?text=${encodeURIComponent('Olá! Vi que estão selecionando campeonatos para usar o PeladaPro de graça. Tenho interesse! Pode me ajudar?')}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 bg-green-500 hover:bg-green-400 text-white font-extrabold text-sm md:text-base px-6 md:px-8 py-3 md:py-3.5 rounded-full transition-all shadow-lg shadow-green-500/40 hover:shadow-green-400/50 hover:scale-105 animate-[pulse_2s_ease-in-out_infinite]"
                >
                  <svg viewBox="0 0 24 24" className="w-5 h-5 md:w-6 md:h-6 fill-white" xmlns="http://www.w3.org/2000/svg"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                  QUERO PARTICIPAR
                </a>
                <button
                  onClick={() => setPromoBannerVisible(false)}
                  className="text-gray-500 hover:text-white p-1.5 transition-colors rounded-full hover:bg-white/10"
          ...
          {/* Borda brilhante animada no fundo */}
          <div className="h-1 w-full" style={{ background: 'linear-gradient(90deg, #fbbf24, #f59e0b, #eab308, #fbbf24, #f59e0b)' }} />
        </div>
      )}

      {/* Hero Section - Mobile-First Conversion Optimized */}
      <section className={`${promoBannerVisible ? 'pt-44 md:pt-48' : 'pt-24 md:pt-28'} pb-12 px-4 md:pb-16 transition-all duration-300`}>
